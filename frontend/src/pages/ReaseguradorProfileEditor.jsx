...
export default function ReaseguradorProfileEditor() {
  const { t } = useI18n();
  ...
  useEffect(() => {
    api.get("/reasegurador/profile/me").then(({ data }) => {
  ...
  const save = async () => {
    await api.put("/reasegurador/profile", p);
    setSaved(true);
    setTimeout(() => setSaved(false), 2000);
  };

  return (
    <div className="p-8 max-w-5xl mx-auto" data-testid="reasegurador-profile-editor">
      <div className="overline">Mi perfil público</div>
      <h1 className="font-display text-3xl font-semibold tracking-tight mt-1 mb-8">{t("reasegurador.profile_title")}</h1>
      ...
      {company && (
        <div className="rsm-card mb-6 bg-slate-50">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            ...
            {company.rating_agency && (
              <div>
                <div className="overline text-slate-400">Rating</div>
                <div className="font-semibold mt-1">{company.rating_agency} {company.rating_value}</div>
              </div>
            )}
      ...
      <div className="rsm-card mb-6">
        ...
        <textarea
          ...
          placeholder="Describe brevemente tu compañía, especialización y enfoque en el mercado de reaseguro..."
          data-testid="bio-field"
        />
        <div className="text-xs text-slate-400 mt-1 text-right">{p.bio.length}/500</div>
      </div>

      <Toggles title={t("reasegurador.branches")} items={BRANCHES} selected={p.branches} onToggle={(v) => toggle("branches", v)} />
      <Toggles title={t("reasegurador.zones")} items={ZONES} selected={p.geographic_zones} onToggle={(v) => toggle("geographic_zones", v)} />
      <Toggles title={t("reasegurador.languages")} items={LANGS} selected={p.languages} onToggle={(v) => toggle("languages", v)} />
